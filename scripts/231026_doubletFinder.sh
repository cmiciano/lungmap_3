...
#PBS -V
#PBS -A epigen-group
#PBS -e /home/cmiciano/job_outs/Lung/lungmap_3/df.R.e
#PBS -o /home/cmiciano/job_outs/Lung/lungmap_3/df.R.o
...
INPUT_FILE="/projects/ps-epigen/users/cmiciano/Lung/lungmap_3/scripts/231026_LungMap3_30Pool_Multiome_metadata_sp_fi.tsv"
...
OUT_PATH="/oasis/tscc/scratch/cmiciano/Lung/lungmap_3/01_DoubletFinder/"
...
Rscript $Script --inp_h5 $SAMP_FP --lib_rna_id $LIB_RNA_ID --samp_id $SAMP_ID --modality $MODALITY --out $OUT_PATH --skip_df --lib_atac_id $LIB_ATAC_ID --lib_rna_atac_id $LIB_RNA_ATAC_ID --fragpath $FRAG_PATH
#                  # 1         #2      #3           #4         #5       #6         #7             #8
...
for DIR in $OUT_PATH*;do
        ...
        BN=$(basename $DIR);
        echo $BN;
        FOUT="/projects/ps-epigen/users/cmiciano/Lung/lungmap_3/01_DoubletFinder/"
        [[ -d $FOUT ]] || mkdir $FOUT
        FDOUT=$FOUT$BN
        [[ -d $FDOUT ]] || mkdir $FDOUT
        echo $FDOUT
